...
#define KMSPECTRUM 1   
/* 0 is for KARMEN rate only analysis, else spectral info is used */
/******* KARMEN simulation variables ********/
#define MAX_SYS 100
...
/* Using KARMEN spectral information */
double KMchiSpectrum(int exp, int rule, int n_params, 
		     double *x, double *errors, void *user_data)
{
      ...
      double KM_bkgd[26] = { 1.8193,   1.55312,  1.02658,  0.63072,  0.887069, 
			     ...
			     0.133088 };
      double *KMsignal_fit_rates = glbGetSignalFitRatePtr(0, 0);
      double KMsignal_rates[26];
      double KMsignal_norm;
      double chi2 = 0.0;
      int  i,ew_low, ew_high;

      glbGetEnergyWindowBins(0, rule, &ew_low, &ew_high);
...
/* Default density correlation list: Every experiment independent */
  ...
  if(KMSPECTRUM==0)
    {
      glbDefineChiFunction(&KMchi,   1,    "KMchi", NULL);
      glbDefineAEDLVariable("KMBINS",1);
    }
  else
    {
      glbDefineChiFunction(&KMchiSpectrum,   1,    "KMchi", NULL);
