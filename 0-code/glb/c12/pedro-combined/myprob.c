    ...
    double filter_sigma, void *user_data)
{
  ...
  double phb = 0.5*dm*GLB_KM_TO_EV(Lmean+b)/E;
  double tilt = 0.0008 * 197.3269631E-12;

  double denominator = SQR(dm)*GLB_KM_TO_EV(b*(0.375-0.0002*b)+(0.375+0.0002*a)*a);
  double factor1 = dm*( (0.0004*a+0.375)*sin(pha) 
			+ (0.0004*b-0.375)*sin(phb));
  double factor2 = tilt*E*(-cos(pha) + cos(phb));
  ...
  /* if(prob>1.)printf("AHHH!\n"); */

  phase2 = phase*0.02914/0.017675;
  double damping = dm*GLB_KM_TO_EV(0.7E-3)/E; /* CAN BE OPT */
  prob2 = filter_sigma<0 ?		/* see mathematica worksheet */
    1.0 - 2.0*s2*c2*( 1.0 + cos(2*phase2)*exp( -SQR(damping)/8.0 ) ) :
    /* 1.0 - 4.0*s2*c2*SQR(sin(phase2)) : */
    /* 2.0 * c2 * ( c2 + s2*cos(phase2) ) : */
    2.0 * c2 * ( c2 + s2*cos(phase2)/* *exp(-SQR(phase2/filter_sigma/2.)) */ );

  for(i=0; i<2; i++)
    for(j=0; j<2; j++)
      P[i][j] = i==j ? 0.9779*prob + 0.0221*prob2 : 1 - (0.9779*prob + 0.0221*prob2);
