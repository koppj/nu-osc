...
/* Default density correlation list: Every experiment independent */
  ...
  /* Initializing MB */
  chiMB_init(0);		/* 0: 200 MeV, 1: 475 MeV */

  /* Define standard oscillation parameters */
  /* Based on hep-ph/1001.4524 */
  double theta12 = 0*asin(sqrt(.99))/2.;
  /* double theta12 = asin(sqrt(1.0))/2.; */
  double theta13 = 0;
  double bigtheta13 = 0;
  double theta23 = 0;
  double deltacp = 0.;
  ...
  /* glbDefineParams(bf_find, asin(sqrt(0.16))/2., 0, 0, 0, 17.50, 0); */
  glbDefineParams(bf_find, 0*asin(sqrt(0.076))/2., 0, 0, 0, 5.9, 0);
  ...
  /* for(ss2t=3E-4; ss2t <= 1.1E0; ss2t*=1.3) */
  for(ss2t=4E-4; ss2t <= 1.3E0; ss2t*=1.1)
    {
      theta=M_PI/4.;
      if(ss2t<0.999)theta = asin(sqrt(ss2t))/2.;
      glbSetOscParams(true_values,theta,GLB_THETA_12);
      /* for(dm=1E-2; dm <= 110; dm*=1.3) */
      for(dm=1E-2; dm <= 100; dm*=1.1)
  	{
