...
// added DB data point from Neutrino 2014 talk
...
// rate_old, rate_new
// numbers for DC and RENO from Giunti 1608.04096  
double sbl_data[N_SBL_R][2] = {
  ...
  {1.013, 0.943},	  // ROV_2S   
  {0.990, 0.922},	  // ROV_3S   
  {0.000, 0.946},	  // RENO_SBL   
  {0.000, 0.935}	  // DC_SBL (only "new rate" availabel!!)  
};
...
double sbl_err_uncor[N_SBL_R] = {
  ...
  4.95,	  // ROV_2S 
  4.53,	  // ROV_3S 
  2.1,     // RENO_SBL (C. Giunti, 2016)
  1.4     // DC_SBL (C. Giunti, 2016)
};
...
const double sbl_iso_fract[N_SBL_R][NISO] = {
  ...
  {0.569, 0.073, 0.301, 0.056},           // RENO_SBL (C. Giunti 2016)
  {0.511, 0.087, 0.340, 0.062}            // DC_SBL (C. Giunti 2016)
};
...
int init_sbl_reactors(int old_new)
{
  ...
  */
  ...
  // specify here which SBL data points to use
  for(int i = 0; i < N_SBL_R; i++)
  {
#ifdef USE_SBL
...
//  fprintf(stderr, "SBL (n_sbl_react = %d) ", nn);
  ...
  if(old_new == OLD && use_sbl_react[DC_SBL])
    error("init_sbl_reactors: cannot use those exps for old flux!!");
  return nn;
}
...
void set_table_sbl(Param_5nu &p, double cff[NBIN_CHISQ][NPULLS+1])
{
  for(int i = 0; i < N_SBL_R; i++){
    ...
    if(use_sbl_react[i]){
      ...
      // the pulls
      cff[ii][PULL_U235_0] = cff[ii][NPULLS]*rate_pull_coupl.unc(U235, 0, sbl_iso_fract[i]);
      cff[ii][PULL_P239_0] = cff[ii][NPULLS]*rate_pull_coupl.unc(P239, 0, sbl_iso_fract[i]); 
      cff[ii][PULL_P241_0] = cff[ii][NPULLS]*rate_pull_coupl.unc(P241, 0, sbl_iso_fract[i]);       
      cff[ii][PULL_U238]   = cff[ii][NPULLS]*rate_pull_coupl.unc(U238, 0, sbl_iso_fract[i]);
      
      for(int j = 1; j < N_CO_UNC; j++){
...
void print_sbl_data(void)
{
  ...
  for(int i = 0; i < N_SBL_R; i++){
    ...
    //printf("%d %e %e %e %e\n", i+1, l, sbl_data[i][NEW], 
    //       0.01*sbl_err_uncor[i], sqrt(fit.S_data[ii][ii])/sbl_data[i][NEW]);
  }
  exit(0);
  return;
}
