...
//       double w = DB_prob.P(prm, i, j, bin) / norm(basel_DB[i][j]); 
//       w_nd += eff_DB[i] * DAQ_DB[i] * w;

//       const int k = i+4;
//       w = DB_prob.P(prm, k, j, bin) / norm(basel_DB[k][j]); 
//       w_fd += eff_DB[k] * DAQ_DB[k] * w;
//     }
//   }
//   const double b = DB_accidentals[bin] + DB_other_backgr[bin];
//   return DB_data_ND[bin] * w_fd / w_nd + b;
// }
...
void DB_init(void)
{
  ...
  // ND and FD weights for no oscillations (ND weights not needed for Neutrino16 data fit)
  double w_nd = 0.;
