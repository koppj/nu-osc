...
void set_table_chooz(Param_5nu &prm, double cff[NBIN_CHISQ][NPULLS+1])
{
  ...
  for(int i = 0; i < NBIN_CH_PROB; i++){
    for(int r = 0; r < 2; r++){
      ...
      // the pulls
      cff[b][PULL_U235_0] = cff[b][NPULLS] * co[r].c0I[i][U235][0] / co[r].c0[i];
      cff[b][PULL_P239_0] = cff[b][NPULLS] * co[r].c0I[i][P239][0] / co[r].c0[i];
      cff[b][PULL_P241_0] = cff[b][NPULLS] * co[r].c0I[i][P241][0] / co[r].c0[i];
      cff[b][PULL_U238]   = cff[b][NPULLS] * co[r].c0I[i][U238][0] / co[r].c0[i];
      
      for(int k = 1; k  < N_CO_UNC; k++){
