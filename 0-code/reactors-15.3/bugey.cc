...
#ifdef BUG_SPECT_FREE // bin-per-bin free normalization (define in definitions.h)
  int bb = fit.first_bin[BUG_SP];
  int pp = fit.first_pull[BUG_SP] + 1;
  for(int i = 0; i < 25; i++){
    cff[bb+i][pp+i]    = cff[bb+i][NPULLS];
    cff[bb+i+25][pp+i] = cff[bb+i+25][NPULLS];
  }
  // different binning of last detector
  bb += 50;
  int i_bin = 0, i_pull = 0;
  for(int i = 0; i < 50; i++){
    if(i_bin == 5){
      i_bin = 0;
      bb++;
    ...
    cff[bb][pp] += 0.2*cff[bb][NPULLS];
    i_bin++;
    i_pull++;
  }        
#endif // BUG_SPECT_FREE       
  return;
}
...
void bugey_init(const int old_new)
{
  ...
  fprintf(stderr, "[bin-per-bin free normalization in Bugey] ");
  for(int i = 0; i < 25; i++){
    const int p = fit.first_pull[BUG_SP]+1+i; 
    fit.S_pull[p][p] = 1.; // arbitrary value (used with free pull)
    fit.pull_status[p] = FREE;
  }
#else
  
//# define BUGEY_BUMP  
  // put a large (correlated) error for the bins with the bump
  // 5 MeV +/- 3 bins for det 1,2
  // 5 MeV +/- 1 bin  for det 3 (E_positron)  
# ifdef BUGEY_BUMP
  fprintf(stderr, " [increasing error for bump-bins for Bugey spectrum by 0.1] ");
# define NBINS_BUMP 14
  int bins_bump[NBINS_BUMP] = {17, 18, 19, 20, 21, 22,
			       17+25, 18+25, 19+25, 20+25, 21+25, 22+25,
			       57, 58};
  for(int i = 0; i < NBINS_BUMP; i++)
    for(int j = 0; j < NBINS_BUMP; j++)
      fit.S_data[b0+bins_bump[i]][b0+bins_bump[j]] += norm(0.1); // put 10% error
# undef NBINS_BUMP
# endif  
#endif // BUG_SPECT_FREE
