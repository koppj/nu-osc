...
const double kKmUnits = 1000.; //FIXME

//#error "Remove free norm from MINOS code"

#define MINOS_2017_MINIMIZE      // Minimize over nuisance parametersin 2017 analysis?
#define MINOS_2017_N_NUISANCE  4 // # of nuisance parameters in MINOS/MINOS+ 2017 analysis
//#define MINOS_2017_N_NUISANCE  5//FIXME
...
//  getchar();
  ...
  // Return total chi^2
//  printf("# MINOS chi2_CC=%g, chi2_NC=%g, chi2_pull=%g\n",
//         chi2_cc, chi2_nc, chi2_pull); //FIXME
...
// Bypasses GLoBES except for the computation of the oscillation
// probabilities.
// If flags contains MINOS2017_PRINT_RATES, the event rates are printed.
// -------------------------------------------------------------------------
{
  const double L_nd   = 1.04;     // km
  const double L_fd   = 735.0;    // km
  const double rho_nd = 0.0;      // grams/cm^3
  const double rho_fd = 0.0*2.8;      // grams/cm^3 FIXME
...
//  const double sig_dist_mplus = -4.71471682040894180e-01;
  ...
  // FIXME
  const double dmsq32 = glbGetOscParamByName(params, "DM31")
                          - glbGetOscParamByName(params, "DM21");
  const double dmsq21 = glbGetOscParamByName(params, "DM21");
  const double dmsq41 = glbGetOscParamByName(params, "DM41");
  const double th23 = glbGetOscParamByName(params, "TH23");
  const double th13 = glbGetOscParamByName(params, "TH13");
  const double th12 = glbGetOscParamByName(params, "TH12");
  const double th14 = glbGetOscParamByName(params, "TH14");
  const double th24 = glbGetOscParamByName(params, "TH24");
...
//  {
//    double P_nd[3][3];
////FIXME    snu_probability_matrix(P_nd, +1, L_nd / NDCC_TrueNC_minos->GetXaxis()->GetBinCenter(j+1),
////                           1, &L_nd, &rho_nd, 0.3, NULL);
//    snu_probability_matrix(P_nd, +1, L_nd / NDCC_TrueNC_minos->GetXaxis()->GetBinCenter(j+1),
//                           1, &L_nd, &rho_nd, -1.0, NULL);
////FIXME    P_nd_truenc[j]   = P_nd[NU_MU][NU_E] + P_nd[NU_MU][NU_MU] + P_nd[NU_MU][NU_TAU];
////    P_nd_numu[j]     = P_nd[NU_MU][NU_MU];
////    P_nd_beamnue[j]  = P_nd[NU_E][NU_E];
////    P_nd_appnue[j]   = P_nd[NU_MU][NU_E];
////    P_nd_appnutau[j] = P_nd[NU_MU][NU_TAU];
//
//    P_nd_truenc[j]   = 1.0;
//    P_nd_numu[j]     = FourFlavourDisappearanceWeight( 1.0,
//        dmsq32, th23, dmsq21, dmsq43, th12, th13, th14, th24, 0, 0, 0, 0,
//        NDCC_TrueNC_minos->GetXaxis()->GetBinCenter(j+1)*kKmUnits);
//    P_nd_beamnue[j]  = 1.0;
//    P_nd_appnue[j]   = 0.0;
//    P_nd_appnutau[j] = 0.0;
////    double a,b;
////    printf("%10.5g   %10.5g %10.5g\n", L_nd / NDCC_TrueNC_minos->GetXaxis()->GetBinCenter(j+1),
////        a=P_nd[NU_MU][NU_MU], b=P_nd_numu[j]);
////    if (fabs((a-b)/a) > 0.05) getchar();
//  }
////  getchar();
////  for (int j=0; j < n_bins_true_fd; j++)
////  {
////    double P_fd[3][3];
//////FIXME    snu_probability_matrix(P_fd, +1, L_fd / FDCC_TrueNC_minos->GetXaxis()->GetBinCenter(j+1),
//////                           1, &L_fd, &rho_fd, 0.3, NULL);
////    snu_probability_matrix(P_fd, +1, L_fd / FDCC_TrueNC_minos->GetXaxis()->GetBinCenter(j+1),
////                           1, &L_fd, &rho_fd, -1.0, NULL);
////    P_fd_truenc[j]   = P_fd[NU_MU][NU_E] + P_fd[NU_MU][NU_MU] + P_fd[NU_MU][NU_TAU];
////    P_fd_numu[j]     = P_fd[NU_MU][NU_MU];
////    P_fd_beamnue[j]  = P_fd[NU_E][NU_E];
////    P_fd_appnue[j]   = P_fd[NU_MU][NU_E];
...
////  }
  ...
  for (int j=0; j < n_bins_true_nd; j++)
  {
    ...
    LoverE[0] = bc - sample;
    LoverE[1] = bc + sample;
//FIXME    for (int k=0; k < 2; k++)
//      snu_probability_matrix(P_nd[k], +1, L_nd / LoverE[k], 1, &L_nd, &rho_nd, 0.3, NULL);
    for (int k=0; k < 2; k++)
...
//    P_nd_truenc[j]   = 1.0;
//    P_nd_numu[j]     = 0.5 * (
//                          FourFlavourDisappearanceWeight( 1.0,
//                            dmsq32, th23, dmsq21, dmsq43, th12, th13, th14, th24, 0, 0, 0, 0,
//                            LoverE[0]*kKmUnits)
//                        + FourFlavourDisappearanceWeight( 1.0,
//                            dmsq32, th23, dmsq21, dmsq43, th12, th13, th14, th24, 0, 0, 0, 0,
...
//    double a,b;
//    printf("%10.5g   %10.5g %10.5g\n", L_nd / bc,
//        a=P_nd[0][NU_MU][NU_MU], b=P_nd_numu[j]);
//    if (fabs((a-b)/a) > 0.05) getchar();
  }
//  getchar();
  ...
  for (int j=0; j < n_bins_true_fd; j++)
  {
    ...
    LoverE[1] = bc + sample;
//FIXME    for (int k=0; k < 2; k++)
//      snu_probability_matrix(P_fd[k], +1, L_fd / LoverE[k], 1, &L_fd, &rho_fd, 0.3, NULL);
    for (int k=0; k < 2; k++)
...
//    double a,b;
//    printf("%10.5g   %10.5g %10.5g\n", L_fd / bc,
//        a=P_fd[0][NU_MU][NU_MU], b=P_fd_numu[j]);
//    if (fabs((a-b)/a) > 0.05) getchar();
  }
//  getchar();
...
//    printf("pred FD NC %10.5g %10.5g\n", pred_fd_nc_minos[i], pred_fd_nc_mplus[i]);//FIXME
  }
//  printf("\n");//FIXME
  k = 0;
  for (int i=0; i < n_bins_reco_nd_cc; i++)
...
//      if (i==0)//FIXME
//        printf("Accumulating ND CC   %d %d   %10.5g %10.5g %10.5g   %10.5g %10.5g %10.5g\n", j, i,
//               sm_nd_cc_numu_minos[k], P_nd_numu[j], pred_nd_cc_minos[i],
//               sm_nd_cc_numu_mplus[k], P_nd_numu[j], pred_nd_cc_mplus[i]);//FIXME
//      if (j % 200 == 0)
//        getchar();
      k++;
    }
//    printf("pred ND CC %10.5g %10.5g\n", pred_nd_cc_minos[i], pred_nd_cc_mplus[i]);//FIXME
  }
//  printf("\n");//FIXME
  k = 0;
  for (int i=0; i < n_bins_reco_fd_cc; i++)
...
//    printf("pred FD CC %10.5g %10.5g\n", pred_fd_cc_minos[i], pred_fd_cc_mplus[i]);//FIXME
  }
//  printf("\n");//FIXME
  ...
  if (flags & MINOS2017_PRINT_RATES)
  {
    int i;
...
//  for (double E=1.; E <= 100.001; E += 1.0) //FIXME
//  {
//    double P[3][3];
//    double L = 735.;
//    double rho = 0.0;
//    snu_probability_matrix(P, +1, E, 1, &L, &rho, -1.0, NULL);
//    printf("%g %g\n", P[NU_MU][NU_MU],
//           FourFlavourDisappearanceWeight(1.0, dmsq32, th23, dmsq21, dmsq43, th12,
//             th13, th14, th24, 0, 0, 0, 0, L/E*kKmUnits));
//  }
