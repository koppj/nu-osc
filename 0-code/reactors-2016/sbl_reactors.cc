...
double sbl_data[N_SBL_R][2] = {
  ...
  {1.013, 0.943},	  // ROV_2S   
  {0.990, 0.922},	  // ROV_3S   
  {0.000, 0.948}	  // DB_SBL (only "new rate" availabel!!)  
};
...
double sbl_err_uncor[N_SBL_R] = {
  ...
  4.95,	  // ROV_2S 
  4.53,	  // ROV_3S 
  2.3     // DB_SBL (p 19 of Neutrino 2014 talk)
};
...
const double sbl_iso_fract[N_SBL_R][NISO] = {
  ...
  {0.586, 0.076, 0.288, 0.050}            // DB_SBL (p 19 of Neutrino 2014 talk)
};
...
{
  // DB data point given on slide 19 of Neutrino 2014 talk INCLUDES theta13!
  // (Email from Patrick on 09/07/2014)
  // correct DB data point, rescale by their best-fit 3nu data point (slide 18)

  Param_5nu p;
  p.theta[I12] = asin(sqrt(0.31));
  p.theta[I13] = 0.5 * asin(sqrt(0.084));
  p.theta[I14] = p.theta[I15] = 0.;
  p.set_ang();
  p.dmq[0] = 0.;          // should always be zero!
  p.dmq[1] = 7.59e-5;
  p.dmq[2] = 2.44e-3;     // dmqee \approx dmq31
  p.dmq[3] = 1.;
  p.dmq[4] = 2.;

  sbl_data[DB_SBL][NEW] *= rate.prob(p, sbl_L[DB_SBL], sbl_iso_fract[DB_SBL]);
...
//  fprintf(stderr, "SBL (n_sbl_react = %d) ", nn);
  ...
  if(old_new == OLD && use_sbl_react[DB_SBL])
    error("init_sbl_reactors: cannot use DB_SBL and old flux!!");
  return nn;
}
