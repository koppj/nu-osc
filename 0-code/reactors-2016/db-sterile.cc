...
double gl_DmqL_db;
int bin_gl_db;
double DB_accidentals[NBIN_DB], DB_other_backgr[NBIN_DB];
double DB_data[NBIN_DB], DB_data_errors[NBIN_DB], DB_bg_errors[NBIN_DB];
double DB_noosc[NBIN_DB];
double pred_EH1_3f[NBIN_DB], pred_EH2_3f[NBIN_DB], pred_EH3_3f[NBIN_DB];
...
void set_table_DB(Param_5nu &prm, double cff[NBIN_CHISQ][NPULLS+1])
{
  ...
  // Compute FD/ND ratios
  double pred_EH1[NBIN_DB], pred_EH2[NBIN_DB], pred_EH3[NBIN_DB];
  for(int bin = 0; bin < NBIN_DB/2; bin++){
    pred_EH1[bin] = 0.0;
    pred_EH2[bin] = 0.0;
    pred_EH3[bin] = 0.0;
    for(int j = 0; j < NREACT_DB; j++){
      int i;
      for(i = 0; i < NDET_EH1; i++)
        pred_EH1[bin] += eff_DB[i] * DAQ_DB[i] * DB_prob.P(prm, i, j, bin)
                           / norm(basel_DB[i][j]);
      for(; i < NDET_EH1+NDET_EH2; i++)
        pred_EH2[bin] += eff_DB[i] * DAQ_DB[i] * DB_prob.P(prm, i, j, bin)
                           / norm(basel_DB[i][j]);
      for(; i < NDET_DB; i++)
        pred_EH3[bin] += eff_DB[i] * DAQ_DB[i] * DB_prob.P(prm, i, j, bin)
                           / norm(basel_DB[i][j]);
    }
  }
  for(int bin = 0; bin < NBIN_DB/2; bin++){
    cff[fit.first_bin[DB]+bin][NPULLS]
      = (pred_EH2[bin] / pred_EH2_3f[bin]) * (pred_EH1_3f[bin] / pred_EH1[bin]);
    cff[fit.first_bin[DB]+NBIN_DB/2+bin][NPULLS]
      = (pred_EH3[bin] / pred_EH3_3f[bin]) * (pred_EH1_3f[bin] / pred_EH1[bin]);
  }

  // FIXME Fix systematic uncertainties
...
{
  // reading data
  FILE *fp_ratios      = fopen(REACTOR_PATH"Data_DB-sterile2016/ratios.dat","r");
  FILE *fp_data_errors = fopen(REACTOR_PATH"Data_DB-sterile2016/data-errors.dat","r");
  FILE *fp_bg_errors   = fopen(REACTOR_PATH"Data_DB-sterile2016/bg-errors.dat","r");

  for(int i = 0; i < NBIN_DB; i++){
    if(fscanf(fp_ratios, "%*e %le", &DB_data[i]) != 1)
      error("cannot read DB data central values");
    if(fscanf(fp_data_errors, "%*e %le", &DB_data_errors[i]) != 1)
      error("cannot read DB data errors");
    if(fscanf(fp_bg_errors, "%*e %le", &DB_bg_errors[i]) != 1)
      error("cannot read DB background errors");
  }
  fclose(fp_ratios);
  fclose(fp_data_errors);
  fclose(fp_bg_errors);

  // Convert lower edges of error bars to sizes of error bars
  for (int i = 0; i < NBIN_DB; i++){
    DB_data_errors[i] = DB_data[i] - DB_data_errors[i];
    DB_bg_errors[i]   = 1. - DB_bg_errors[i];
  }

  // Compute predictions with only 3-flavor oscillations
  DB_prob.calc_integrals();
  Param_5nu p3f;
  p3f.theta[0] = NAN;
  p3f.theta[I12] = asin(sqrt(0.31));
  p3f.theta[I13] = 0.5*asin(sqrt(0.084));
  p3f.theta[I14] = 0.0;
  p3f.theta[I15] = 0.0;
  p3f.set_ang();
  p3f.dmq[0]     = 0.;
  ...
  for(int bin = 0; bin < NBIN_DB/2; bin++){
    ...
    for(int j = 0; j < NREACT_DB; j++){
      int i;
      for(i = 0; i < NDET_EH1; i++)
        pred_EH1_3f[bin] += eff_DB[i] * DAQ_DB[i] * DB_prob.P(p3f, i, j, bin)
                              / norm(basel_DB[i][j]);
      for(; i < NDET_EH1+NDET_EH2; i++)
        pred_EH2_3f[bin] += eff_DB[i] * DAQ_DB[i] * DB_prob.P(p3f, i, j, bin)
                              / norm(basel_DB[i][j]);
      for(; i < NDET_DB; i++)
        pred_EH3_3f[bin] += eff_DB[i] * DAQ_DB[i] * DB_prob.P(p3f, i, j, bin)
                              / norm(basel_DB[i][j]);
    }
  }
  ...
  // expected number of accidentals in each detector
  // (assuming the same relative magnitude of the BG -- we could do better here)
  double sum_bg_eh1=0.0, sum_bg_eh2=0.0, sum_bg_eh3=0.0;
  double sum_uncert_sq_eh1=0.0, sum_uncert_sq_eh2=0.0, sum_uncert_sq_eh3=0.0;
  for(int i = 0; i < NDET_EH1; i++){
    sum_bg_eh1        += bg_DB_data[0][i] * DAQ_DB[i] * eff_DB[i];
    sum_uncert_sq_eh1 += norm(bg_err_DB[0][i] * DAQ_DB[i] * eff_DB[i]);
  }
  for(int i = NDET_EH1; i < NDET_EH1+NDET_EH2; i++){
    sum_bg_eh2        += bg_DB_data[0][i] * DAQ_DB[i] * eff_DB[i];
    sum_uncert_sq_eh2 += norm(bg_err_DB[0][i] * DAQ_DB[i] * eff_DB[i]);
  }
  for(int i = NDET_EH1+NDET_EH2; i < NDET_DB; i++){
    sum_bg_eh3        += bg_DB_data[0][i] * DAQ_DB[i] * eff_DB[i];
    sum_uncert_sq_eh3 += norm(bg_err_DB[0][i] * DAQ_DB[i] * eff_DB[i]);
  }
...
//    //DB_data_ND[i] *= 0.999; // introduced "by hand" to match th13 best fit point for Neutrino14 fit
    ...
    const int b = fit.first_bin[DB] + i;  
    fit.Data[b]      = DB_data[i]; 
    fit.S_data[b][b] = norm(DB_data_errors[i]) + norm(DB_bg_errors[i]);
  }
  ...
  // accidental background
  fit.S_pull[p][p] = sum_uncert_sq_eh3 / norm(sum_bg_eh3);  //FIXME More syst. errors here?
  p++;
