...
/*********************************************
 * using results presented at Moriond 2016
 *********************************************/
...
void dc_init(const int old_new)
{
  ...
  // first 27 data points correspond to FD-I data
  FILE *fp_data      = fopen(REACTOR_PATH"Data_DC-moriond-2016/Data-FD-I.csv","r");
  FILE *fp_data_err  = fopen(REACTOR_PATH"Data_DC-moriond-2016/Data-FD-I-lower.csv","r");
  FILE *fp_sys_e     = fopen(REACTOR_PATH"Data_DC-moriond-2016/blue-hist-FD-I.csv","r");
  FILE *fp_sys       = fopen(REACTOR_PATH"Data_DC-moriond-2016/yellow-hist-FD-I.csv","r");
  //FILE *fp_sys       = fopen(REACTOR_PATH"Data_DC-moriond-2016/green-hist-FD-I.csv","r");
  ...
  // second 27 data points correspond to FD-II/ND data
  fp_data      = fopen(REACTOR_PATH"Data_DC-moriond-2016/Data.csv","r");
  fp_data_err  = fopen(REACTOR_PATH"Data_DC-moriond-2016/Data-low.csv","r");
  fp_sys_e     = fopen(REACTOR_PATH"Data_DC-moriond-2016/blue-hist.csv","r");
  fp_sys       = fopen(REACTOR_PATH"Data_DC-moriond-2016/green-hist.csv","r");
  ...
  for(int p = 0; p < NPULL_DC; p++){
    int pp = fit.first_pull[DC] + p;
    fit.S_pull[pp][pp] = 1.;
  }
  const int pp = fit.first_pull[DC] + 6;
  fit.S_pull[pp][pp] = norm(0.004);
  fit.pull_status[pp] = ACTIVE;

  // correlation of reactor uncert between FD-I and FD-II (slide 15)
  fit.S_pull[fit.first_pull[DC] + 2][fit.first_pull[DC] + 4] = 
    fit.S_pull[fit.first_pull[DC] + 4][fit.first_pull[DC] + 2] = 0.72; 
  fit.S_pull[fit.first_pull[DC] + 5][fit.first_pull[DC] + 3] = 
    fit.S_pull[fit.first_pull[DC] + 3][fit.first_pull[DC] + 5] = 0.72; 
