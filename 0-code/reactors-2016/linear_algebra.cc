...
{
  typedef double MY_VEC[size];
  typedef double MY_ARR[size][size];

  MY_ARR A;
  MY_ARR &B = *(MY_ARR *) B_in;
  MY_ARR &N = *(MY_ARR *) N_out;
  MY_VEC &L = *(MY_VEC *) L_out;
...
bool singsolve(const int size, void *B_in, void *L_out)
{
   ...
   typedef double MY_VEC[size];
   MY_VEC &L = *(MY_VEC *) L_out;   
   typedef double MY_ARR[size][size1];
   MY_ARR &B = *(MY_ARR *) B_in;
