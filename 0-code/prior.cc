...
// -------------------------------------------------------------------------
// In the lightweight version, we've removed:
//   * various deprecated versions of codes (e.g. old MiniBooNE)
//   * MINOS (to avoid dependency on ROOT)
//   * Michele's solar and atm codes (as we only have them in binary format and
//     can't recompile them on new platforms)
//   * Thomas' SBL codes because the linear algrebra library that comes with
//     them doesn't compile on clang due to type conversion issues
// -------------------------------------------------------------------------
...
//
//    gsl_complex _U[n_flavors][n_flavors];
//    memcpy(_U, gsl_matrix_complex_ptr(snu_get_U(),0,0),sizeof(_U[0][0])*SQR(n_flavors));
//    // the following doesn't work in clang, only in gcc
