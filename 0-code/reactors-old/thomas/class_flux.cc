...
double global_flux(const int iso, const double Enu, const int old_new)
{
  ...
  switch(iso){
    ...
    case U235: return read("Dat/Patrick-U235-flux.dat", Enu);
    case P239: return read("Dat/Patrick-Pu239-flux.dat", Enu);
    case P241: return read("Dat/Patrick-Pu241-flux.dat", Enu);
...
double int_corr(double eNu){
  ...
  switch(iso_gl_init){
  case U235:
    w = read("Dat/Patrick-U235-err_cor.dat", eNu);
    break;
  case P239:
    w = read("Dat/Patrick-Pu239-err_cor.dat", eNu);
    break;
  case P241:
    w = read("Dat/Patrick-Pu241-err_cor.dat", eNu);
    break;
  default:
    w = -1.;
  }
...
void init_fluxes(void)
{
   const char *filename_old[NISO] = 
     {"Dat/U235.dat", "Dat/U238.dat", "Dat/Pu239.dat", "Dat/Pu241.dat"};
   const char *filename_new[NISO] = 
     {"Dat/U235_patrick.dat", "Dat/U238_new.dat", "Dat/Pu239_patrick.dat", "Dat/Pu241_patrick.dat"};
...
void calc_cs_per_fission(void)
{
  ...
  for(int i = 0; i < 25; i++){
    ...
    const double EposKin=e-ME-DELTA;
    unc[0] +=  norm(0.25 * read("Dat/Patrick-U235-err_unc.dat", e)  * global_flux(U235, e, NEW) * crossSect(EposKin));
    unc[1] +=  norm(0.25 * read("Dat/Patrick-Pu239-err_unc.dat", e) * global_flux(P239, e, NEW) * crossSect(EposKin));
    unc[2] +=  norm(0.25 * read("Dat/Patrick-Pu241-err_unc.dat", e) * global_flux(P241, e, NEW) * crossSect(EposKin));
  }
  for(int i = 0; i < 3; i++){
  ...
  const char *name[3] = 
    {"Dat/Patrick-U235-err_cor.dat","Dat/Patrick-Pu239-err_cor.dat","Dat/Patrick-Pu241-err_cor.dat"};
