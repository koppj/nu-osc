...
double fold_dc_corr(double eNu0)
{
  const double eNu = eNu_corr(eNu0);
  iso_gl_dc = U235;
  double w = read("Dat/Patrick-U235-err_cor.dat", eNu) * dc_flux(eNu);
  iso_gl_dc = P239;
  w += read("Dat/Patrick-Pu239-err_cor.dat", eNu) * dc_flux(eNu);
  iso_gl_dc = P241;
  w += read("Dat/Patrick-Pu241-err_cor.dat", eNu) * dc_flux(eNu);
