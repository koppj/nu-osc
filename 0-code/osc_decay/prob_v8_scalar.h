...
namespace regeneration{
  ...
  public:
    ...
    double get_g(double m4Gamma){
      ...
      return m4GammaOverG2 > 0. ? sqrt(m4Gamma/m4GammaOverG2) : 0;
    }

    // Returns the nu_4 -> nu_j **rest frame** partial width
    double getGamma(int j) const{
      if (m_A+m_nu[j] >= m_nu[3]) // enforce kinematic constraint
        return 0.0;
  ...
  private:
    ...
    */
    ...
    double getIntegral(double edaughter, double L, const marray<cmplx, 4> &inirho,
      ...
      double result = 0.;
      for(uint j= 0; j < 3; ++j){
        if (prm.m_A + prm.m_nu[j] > prm.m_nu[3]) // enforce kinematic constraint
          continue;
      ...
      // if no \nu_4 have decayed (e.g. because of kinematic constraints), we can stop here
      if (result <= 0.)
        return 0.;
