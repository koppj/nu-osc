...
namespace regeneration{
  ...
  public:
    ...
    double dmq41; /* eV^2 */
    std::array<double, 4> m_nu; /* Vector of neutrino masses (eV) */
    ...
    double get_g(double m4Gamma){
      ...
      for(uint j=0; j<3; ++j){
        if (m_A+m_nu[j] >= m_nu[3]) // enforce kinematic constraint (JK)
            continue;
        double g_4j = SQR(cabs(U[3][j] * U[3][3]));
    ...
    inline double getGammaPhi() const{
      double Usq = SQR(cabs(U[3][3]*U[3][3]));
  ...
  private:
    ...
    inline double interpolateState(double ***state, double E, uint i_nu, uint i_fla) const{
      ...
      uint k = 1;
      while (E_range[k] < E && k < E_range_size-1)
        k++;
    ...
    cmplx getIniRho(double E, uint i_nu, uint i, uint j) const{
      cmplx res = 0;
      for(int i_fla=0; i_fla<4; ++i_fla)
	res += (i_nu==0)? conj(prm.U[i_fla][i])*prm.U[i_fla][j] * interpolateState(inistate, E, i_nu, i_fla) :
	  prm.U[i_fla][i]*conj(prm.U[i_fla][j]) * interpolateState(inistate, E, i_nu, i_fla);
    ...
    */
    double spectrum(double eparent, double L, uint j, double edaughter, uint i_nu) const{
      ...
      double res = 0;
      // Interpolate
      double rho44 = creal(getIniRho(eparent, i_nu, 3, 3));
      uint i_nu_other = (i_nu==0)? 1 : 0;
      double rho44_other = creal(getIniRho(eparent, i_nu_other, 3, 3));
    ...
    double getPhiDecayIntegrand(double Eq, double E, double L) const{
      ...
      double rho44_nu    = creal(getIniRho(Eq, 0, 3, 3));
      double rho44_nubar = creal(getIniRho(Eq, 1, 3, 3));
  ...
  public:
    ...
    regProb(marray<double,1> E_range_, marray<double, 3>& inistate_, bool majorana_):
      ...
      inistate = new double**[E_range_size];
      for(uint i=0; i<E_range_size; ++i){
        inistate[i] = new double*[2];
        for(int j=0; j<2; ++j)
	  inistate[i][j] = new double[4];
      }
