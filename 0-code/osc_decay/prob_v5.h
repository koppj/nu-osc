...
namespace regeneration{
  ...
  public:
    ...
    double m_A; /* Mass of the vector boson (eV) */
    double g;
	...
	if(xj4 > 1.0e-6)
	  m4GammaOverG2 += m_nu[3] * g_4j*g_4j*m_nu[3]/(8.0*M_PI) *
	    ( (ALPHA + 4*xj4)*sqr + //Transverse part
	      0.5*pow((1+xj4)/xA4, 2)* ( (ALPHA - 2*xj4)*sqr +
					 2*xj4_sq*log(ALPHA*(ALPHA+sqr)/(2*xj4_sq) - 1) ) //Goldstone part 
	      );
	else //Put x_j4 inside the log so that 0**0 = 1 and there is no singularity
	  m4GammaOverG2 += m_nu[3] * g_4j*g_4j*m_nu[3]/(8.0*M_PI) *
	    ( (ALPHA + 4*xj4)*sqr + //Transverse part
	      0.5*pow((1+xj4)/xA4, 2)* ( (ALPHA - 2*xj4)*sqr +
					 2*log(pow(2*xj4_sq/(ALPHA*ALPHA - 2*xj4_sq - ALPHA*sqr), xj4_sq)) ) //Goldstone part 
	      );
      }
  ...
  private:
    ...
    const bool majorana;
    // Pseudovector (true) or vector (false) coupling
    const bool pvector;
    ...
    void compute_Gammas(){
      for(uint j=0; j<3; ++j) {
	double g_4j = prm.g_4j(j);
	double xj4 = prm.xj4(j);
	double xA4 = prm.xA4();
	...
	if(xj4 > 1.0e-6)
	  gamma[j] = g_4j*g_4j*prm.m_nu[3]/(8.0*M_PI) *
	    ( (ALPHA + 4*xj4)*sqr + //Transverse part
	      0.5*pow((1+xj4)/xA4, 2)* ( (ALPHA - 2*xj4)*sqr +
					 2*xj4_sq*log(ALPHA*(ALPHA+sqr)/(2*xj4_sq) - 1) ) //Goldstone part 
	      );
  ...
  public:
    ...
    //Returns the LAB FRAME ChP decay spectrum
    inline double dGdELL(uint j, double eparent, double edaughter) const{
      ...
      return g_4j*g_4j*prm.m_nu[3]*prm.m_nu[3]/(8.0*M_PI*eparent*eparent) *
	( ALPHA + 2*xj4 - eparent/edaughter*xj4_sq - edaughter/eparent +
	  0.5*pow((1+xj4)/xA4, 2)*pow(edaughter/eparent - xj4, 2)*eparent/edaughter
	  );
    }

    //Returns the LAB FRAME ChV decay spectrum
	...
	( pow(edaughter/eparent + xj4, 2)*eparent/edaughter +
	  0.5*pow((1+xj4)/xA4, 2)*(ALPHA - edaughter/eparent - eparent/edaughter*xj4_sq)
	  );
    }
  private:
  ...
  public:
     ...
     *                     i_fla==2: [anti]nu_tau
     *                     i_fla==3: [anti]nu_s
     *  * pvector_: pseudovector (true) or vector (false) decay
     *  * majorana_: whether neutrinos are Majorana or not.
     *               If true, there is a chirality-flipping regeneration contribution
     */
    regProb(marray<double,1> E_range_, marray<double, 3>& inistate_, bool pvector_, bool majorana_):
      majorana(majorana_), pvector(pvector_),
     ...
     * the object
     */
    regProb(bool pvector_, bool majorana_):
      majorana(majorana_), pvector(pvector_){}
  ...
  inline marray<double,1> linspace(double Emin,double Emax,unsigned int div){
    if(div==0)
