...
namespace regeneration{
  ...
  public:
    ...
    inline double g_4j(uint j) const{
      return g * (SQR(creal(U[3][j])) + SQR(cimag(U[3][j])))
               * (SQR(creal(U[3][3])) + SQR(cimag(U[3][3])));
    }
...
//      printf("getIntegral: %20.15g %20.15g\n", result, result_phi_decay);//FIXME
...
//      printf("getfinalflux : %10.5g %10.5g %10.5g   %10.5g %10.5g\n",
//          finstate[0][0], finstate[0][1], finstate[0][2], E, L);//FIXME
//      getchar();//FIXME
      return finstate;
    }
