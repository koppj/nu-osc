...
namespace regeneration{
  ...
  public:
    ...
    inline double xj4_sq(uint j) const{
      return SQR(m_nu[j]/m_nu[3]);
    }
    inline double xA4_sq() const{
      return SQR(m_A / m_nu[3]);
    }
    ...
    // FIXME JK check if I'm using the correct combination of mixing matrix elements
    inline double getGammaPhi() const{
      double Usq = std::abs(U[3][3]*U[3][3]);
      return g*g * m_A / (8.*M_PI) * SQR((1. - Usq)*Usq);
    }
  ...
  private:
    ...
    inline cmplx interpolateRho(const marray<cmplx, 4> &rho, double E, uint i_nu, uint i, uint j) const{
      ...
      if (E < 0.9*E_range[0] || E > 1.1*E_range[E_range.size()-1])
        return 0.0;

      uint k = 1;
      while (E_range[k] < E && k < E_range.size()-1)
        k++;
      double t = (E - E_range[k-1]) / (E_range[k] - E_range[k-1]);
      return (1.-t) * rho[k-1][i_nu][i][j] + t * rho[k][i_nu][i][j];

      // FIXME JK I'm pretty sure there was a bug here in case E > E_max
...
//      return f1*state[xid][i_nu][i_fla] + f2*state[xid+1][i_nu][i_fla];
    ...
    */
    ...
    double getPhiDecayF(double Eq, double t, double L) const{
      ...
      double a_phi = m_phi * prm.getGammaPhi() * L;
      double a_4   = prm.m_nu[3] * prm.getGamma() * L;
//      printf("aphi=%g, a4=%g\n", a_phi, a_4);//FIXME
      return gsl_sf_expint_Ei(-a_phi/t)  +  log(t/Eq)
           - exp(-a_4/Eq) * (gsl_sf_expint_Ei(a_4/Eq - a_phi/t) - log(fabs(a_phi/t - a_4/Eq)));
      // FIXME typo in eq. 30: \Gamma_4 -> \Gamma_4^0, same for \Gamma_\phi
      // FIXME typo in eq. 30: fabs in log is missing
    }

    //! Returns the integrand of the Eq integrals in eq. (29)
    double getPhiDecayIntegrand(double Eq, double E, double L,
                                const marray<cmplx,4> &inirho) const{
      double x_phi4_sq = prm.xA4_sq();
      double rho44_nu    = std::real(interpolateRho(inirho, Eq, 0, 3, 3));
      double rho44_nubar = std::real(interpolateRho(inirho, Eq, 1, 3, 3));
//      printf("          phidecayint  %7.3g %7.3g %7.3g\n",
//             rho44_nu + rho44_nubar,
//             getPhiDecayF(Eq, Eq, L),
//             getPhiDecayF(Eq, E,  L));//FIXME FIXME
    ...
    double getIntegral(double edaughter, double L, const marray<cmplx, 4> &inirho,
                       uint i_nu) const{
...
//      printf("F = %g %g\n", getPhiDecayF(1.*units::GeV, 1.*units::GeV, L, inirho),
//                            getPhiDecayF(1.*units::GeV, 0.5*units::GeV, L, inirho));//FIXME

      // FIXME Ivan has an extra factor 0.75 in front of the regeneration integral - why?
      ...
      for(int i_E = 0; i_E<8; ++i_E){
        ...
        result_phi_decay += A*w16[i_E] * (getPhiDecayIntegrand(Eq1,edaughter,L,inirho)
                                        + getPhiDecayIntegrand(Eq2,edaughter,L,inirho));
      }
//      printf("gamma1 = %g\n", result_phi_decay);//FIXME
...
//        printf("    gamma2 int %3d %15.10g %15.10g\n",
//               i_E, getPhiDecayIntegrand(Eq1,x_phi4_sq*Eq1,L,inirho)*edaughter/(t1*t1),
//               getPhiDecayIntegrand(Eq2,x_phi4_sq*Eq2,L,inirho)*edaughter/(t2*t2));//FIXME
        result_phi_decay += A*w16[i_E]
             * (getPhiDecayIntegrand(Eq1,x_phi4_sq*Eq1,L,inirho)*edaughter/(t1*t1)
              + getPhiDecayIntegrand(Eq2,x_phi4_sq*Eq2,L,inirho)*edaughter/(t2*t2));
      }
//      printf("gamma2 = %g\n", result_phi_decay);//FIXME
...
//      printf("+++T %g\n", getIntegral(0.2*units::GeV, L, inirho, 0)); //FIXME
//      getchar();//FIXME
