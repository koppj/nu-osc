...
namespace regeneration{
  ...
  private:
    ...
    inline double interpolateState(const marray<double, 3> & state, double E, uint i_nu, uint i_fla) const{
	...
	res += (i_nu==0)? std::conj(prm.U[i_fla][i])*prm.U[i_fla][j] * interpolateState(inistate, E, i_nu, i_fla) :
	  prm.U[i_fla][i]*std::conj(prm.U[i_fla][j]) * interpolateState(inistate, E, i_nu, i_fla);
      return res;
    }
    ...
    */
    double spectrum(double eparent, double L, uint j, double edaughter, uint i_nu) const{
      ...
      double res = 0;
      // Interpolate
      double rho44 = std::real(getIniRho(eparent, i_nu, 3, 3));
      uint i_nu_other = (i_nu==0)? 1 : 0;
      double rho44_other = std::real(getIniRho(eparent, i_nu_other, 3, 3));
    ...
    double getPhiDecayIntegrand(double Eq, double E, double L) const{
      ...
      double rho44_nu    = std::real(getIniRho(Eq, 0, 3, 3));
      double rho44_nubar = std::real(getIniRho(Eq, 1, 3, 3));
  ...
  public:
     ...
     */
    marray<double, 2> getFinalFlux(double E, double L, const double filter_value=0.){
      ...
      for(uint i_nu = 0; i_nu<2; ++i_nu){
          ...
          if(prm.g > 1.0e-7)
            finrho[i_nu][3][j] = getIniRho(E, i_nu, 3, j) *
              std::polar(exp(-prm.m_nu[3] * totalGamma * L /(2*E) ),
                         -prm.dmq41 * L /(2.*E)); // JK include GLoBES-style low-pass filter
          else
            finrho[i_nu][3][j] = getIniRho(E, i_nu, 3, j) *
              std::polar(exp(-0.5 * SQR(filter_value) / SQR(E) * SQR(L*0.5*prm.dmq41/E)),
                         -prm.dmq41 * L /(2.*E));
        }
        ...
        for(int i=0; i<3; ++i){ // JK
          for(int j=0; j<=i; ++j){ // JK - implement oscillations and GLoBES-style low-pass filter
            ...
            finrho[i_nu][i][j] *= std::polar(exp(-0.5 * SQR(filter_value) / SQR(E)
                                                    * SQR(L*0.5*dmsq/E)), -dmsq * L /(2.*E));
          }
        }
